...
def evaluate_training_set(training_set:TrainingSet, All_LM_features, depth = 30, k = 5):
    ...
    val_labels = [training_set.scores[i, j] for i, j in itertools.product(val_index, first_k)] 
    train_dataset = PromptWithLMFeaturesDataset(training_set.features[train_index], LM_features, labels)
    val_dataset = PromptWithLMFeaturesDataset(training_set.features[val_index], LM_features, val_labels)
    # print(first_5)
    ...
    # print(train_dataset.data.shape,len(train_dataset.labels),val_dataset.data.shape, len(val_dataset.labels))
    model.fit(
        train_dataset.data, train_dataset.labels,
        eval_set=[(val_dataset.data, val_dataset.labels)],
        verbose = 1
    )
